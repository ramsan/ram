...
def b(e)
var =e.email#'rameshram535@gmail.com'# enter admin email id
...
@msg = Array.new
  if ( !memoryid.nil? && !userid.nil? )
      ...
      if(countmemuser > 0)
        ...
        @msg[0]= {"result"=>"user unfavourite successfully","favcount"=>countuserids}
      else
...
render :json => @msg
 ...
 render :json => @msg
  ...
  catmem1 = Categories_memorie.find(:all,:conditions =>["category_id=?",cat.id])
  catmem1.each do |c|
...
msg4[k]={"categoryId"=>catid.to_s,"categoryName"=>cat.name.to_s,"memorieId"=>mem.id.to_s,"memorieName"=>mem.name.to_s,"memorieView"=>mem.views.to_s,"memoriedecade"=>mem.decade.to_s,"MemorieImage"=>imgpic.to_s,"MemoryCreatedDate"=>mem.created_at.to_s,"UserId"=>usr.id.to_s,"UserName"=>usr.first_name.to_s,"coomentscount"=>comentcount.to_s,"followscount"=>comentcount.to_s,"status"=>status.to_s,"description"=>mem.description.to_s,"favstatus"=>fstatus.to_s,"favcount"=>fcount1.to_s}
 ...
 def followuser
 followeeid = 151#params[:followeeid]
 followerid = 157#params[:followerid]
 @msg = Array.new
...
@msg[0]={"message"=>"user unfollowed successfully"}
...
@msg[0]={"message"=>"please provide the values"}
 ...
 render :json => @msg
  ...
  msg1 = Array.new
  userid = 157#params[:userid]
  catid = 1#params[:catid]
  search1 ='Devo'# params[:search]
  if search1.length != 0
  i=0
  d1 =Category.find_by_id(catid)
  if  !d1.nil?
